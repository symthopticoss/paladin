...
module.exports.load = async function(app, db) {
  ...
  app.get("/buyram", async (req, res) => {
    ...
    if (newsettings) {
      ...
      if(newram > 6938489392) return res.send("You reached max ram limit!");
      if (newusercoins == 0) {
  ...
  app.get("/buydisk", async (req, res) => {
    ...
    if (newsettings) {
      ...
      if(newdisk > 32003748380) return res.send("You reached max disk limit!");
      if (newusercoins == 0) {
        await db.delete("coins-" + req.session.userinfo.id);
  ...
  app.get("/buycpu", async (req, res) => {
    ...
    if (newsettings) {
      ...
      if(newcpu > 3200378300) return res.send("Reached max CPU limit!");
      if (newusercoins == 0) {
        await db.delete("coins-" + req.session.userinfo.id);
  ...
  app.get("/buyservers", async (req, res) => {
    ...
    if (newsettings) {
      ...
      if(newservers > 32037838000) return res.send("Reached max server limit!");
      if (newusercoins == 0) {
        await db.delete("coins-" + req.session.userinfo.id);
