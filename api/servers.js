...
async function ipFromSrv(id) {
        console.log("ID "+id)
    let allocz = (await axios.get(`https://panel.hival.net/api/client/servers/${id}/network/allocations`,{headers:{'Authorization':`Bearer ptlc_JgwisbBaQF283jWADs3mc34WY3vtJv7c3BthATZLBQl`}})).data.data;
    for(let i = 0; i < allocz.length; i++) {
        if(allocz[i].attributes.is_default == true) {
            let ip = (allocz[i].attributes.ip_alias ? allocz[i].attributes.ip_alias : allocz[i].attributes.ip)+":"+allocz[i].attributes.port;
            return ip;
        }
    }
}
function installSrv(id,name) {
 setTimeout(async () => {
     let ip = await ipFromSrv(id);
     axios.get("http://network-1.hival.net:8003/addServer?ip="+ip.split(":")[0]+"&port="+ip.split(":")[1]+"&name="+name+"&token=HivalCloudToken")
 }, 65000)   
}
      ...
      queue.addJob(async (cb) => {
        ...
        if (req.query.name && req.query.ram && req.query.disk && req.query.cpu && req.query.egg && req.query.location) {
          ...
          if (!isNaN(ram) && !isNaN(disk) && !isNaN(cpu)) {
            ...
            setTimeout(async () => {
                cb()
              installSrv(serverinfotext.attributes.identifier,name)
